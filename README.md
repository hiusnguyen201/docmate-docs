...
| **UC Specs** | `specs/README.md` | Quy ước format UC + bản đồ 5 hộp (Auth, SessionChat, Ingestion, Sources, RagChat) |
| | `specs/<Hộp>/UC-*.md` | 16 use case spec — hợp đồng hành vi để code theo (spec-driven) |
...
- **Trước khi code một story:** hàng tương ứng trong `15-traceability-matrix` → **UC spec trong `specs/`** (tra hộp tại `specs/README`) → AC trong `03` → test case trong `12` → mục docs kỹ thuật đặc thù.
