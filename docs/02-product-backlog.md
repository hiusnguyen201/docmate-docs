...
| **E1** | Auth & Account | Đăng ký/đăng nhập Google + email/pass, JWT dùng chung Java/Golang | 17 |
...
**Must = 89 SP** (~5,5 sprint) · Must + Should = 109 SP (~7 sprint). Đề xuất: **v1.0 = toàn bộ Must**, các Should phát hành ở sprint kế tiếp.
...
| US-1.5 | As a user, I want xóa tài khoản và toàn bộ dữ liệu (phiên, nguồn, embedding, hội thoại), so that tôi toàn quyền với dữ liệu cá nhân. | Should | 3 |
...
| Sau v1 | Should/Could còn lại | US-1.4, US-1.5, US-2.5, US-2.6, US-2.7 | 15 |
